...
public sealed class GetUserInfoByUsernameQueryHandler(IUserRepository userRepository) : IRequestHandler<GetUserInfoByUsernameQuery, Result>
{
    public async Task<Result> Handle(GetUserInfoByUsernameQuery request, CancellationToken cancellationToken)
    {
